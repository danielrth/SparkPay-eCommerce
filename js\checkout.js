...
$(function () {
  ...
  window.focusedInputSelector = undefined;
  ...
  // assign the currently focused item to the focusedInputSelector global.
  // var grabfocusedInputSelector = function () {
  //   focusedInputSelector = buildFocusSelector($(":focus")[0]);
  // };
  
  var buildFocusSelector = function (el) {
    return el.nodeName + (el.id ? "#" + el.id : "") + (el.className ? "." + el.className.trim().split(' ').join('.') : "");
  };
  ...
  var checkoutEvents = function () {
	...
	if (selectedType == "Custom")
        window.selectedCustomPayment = paymentSelect.find("option:selected").attr("paymenttypename");
      else
        window.selectedCustomPayment = undefined;
  ...
  var triggerInputs = function () {
    ...
    $("textarea, input[type=text], input[type=password], select").each(function(i, el){
  ...
  var labelHandler = function (e) {
    ...
    if (eventType == "focusin") {
      ...
      focusedInputSelector = buildFocusSelector(el);
  ...
  //prm.add_beginRequest(grabfocusedInputSelector);
  prm.add_endRequest(checkoutEvents);
  
  // attach events
  $(document)
  .on("focus blur change", "textarea, input[type=text], input[type=password], select", labelHandler)
  .on("focusin focus", "select input textarea", function (e) {
    focusedInputSelector = buildFocusSelector(e.target);
  });
  $(document).on("click", ".payment-name", handlePaymentSwitch);
